...
def hello_world(request):
    ...
    if serializer.is_valid():
        ...
        return Response(
            {"error": "Data tidak valid"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def get_users(request):
    ...
    response_data = {
        ...
        "users": USERS_DB,
        "page_info": {"current_page": 1, "total_pages": 1, "has_next": False},
    }
...
def create_user(request):
    ...
    return Response(
        {"success": False, "message": "Data tidak valid", "errors": serializer.errors},
        status=status.HTTP_400_BAD_REQUEST,
    )
...
def get_user_by_id(request, user_id):
    ...
    try:
        ...
        return Response(
            {"success": False, "message": f"User dengan ID {user_id} tidak ditemukan"},
            status=status.HTTP_404_NOT_FOUND,
        )

    except ValueError:
        return Response(
            {"success": False, "message": "ID user harus berupa angka"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def delete_user(request, user_id):
    ...
    try:
        ...
        user_index = next(
            (i for i, u in enumerate(USERS_DB) if u["id"] == int(user_id)), None
        )
